...
public class Utils {
    ...
    public static String dataToHexCardId(byte[] data) {
        if (data.length > 7) {
            try {
                String s = Utils.bytesToHex(Arrays.copyOfRange(data, 6, 14));
                return String.format("H%s",s.replace(" ","").replace("0x",""));
            } catch (Exception e) {
                return null;
            }
        } else {
...
//        byte[] bytes = hexStringToByte("1D");
//        System.out.println(bytes[0]);
//        String s = conver2BinStr(bytes);
//        System.out.println(s);
    ...
    public static String conver2BinStr(byte[] b) {
